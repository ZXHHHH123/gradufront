...
class bossInfoDetail extends Component {
  constructor(props) {
    super(props);
    this.state = {
      userImg: '',//用户头像，进入界面获取用户基本信息然后得到用户的头像赋值给该变量，如果没有就默认一个头像
  ...
  saveBossInfo() {
    ...
    let isSave = this.state.userImg && this.state.userName && this.state.userCompany && this.state.userPlace && this.state.creditReverseSide && this.state.creditFrontSide && this.state.userCreditCode;
    ...
    console.log(formData);
    axios.post(url + 'user/submitBossInfo', formData, {
      headers: {
        ...
        <View style={styles.bossInfoDetail_box}>
          ...
          <Flex style={styles.bossInfoDetail_box_credit_items}>
            <Flex direction="column" style={styles.bossInfoDetail_box_credit_item}>
              <Text style={{fontSize: 16}}>身份证正面</Text>
            ...
            <Flex direction="column" style={styles.bossInfoDetail_box_credit_item}>
              <Text style={{fontSize: 16}}>身份证反面</Text>
              <TouchableOpacity onPress={this.pickSingleWithCamera.bind(this, 'creditReverseSide')}>
                {!this.state.creditReverseSide ?
                    ...
                    <Image
                        style={styles.bossInfoDetail_box_credit_photo} source={{uri: this.state.creditReverseSide.uri}}/>
                }
              </TouchableOpacity>
